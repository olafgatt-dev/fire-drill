...
export default function App() {
  // Navigation: "setup" | "drill" | "marshals" | "employees"
    ...
    (async () => {
      ...
      try {
        ...
        const { data: sData } = await supabase
          .from("drill_sessions").select("*").order("started_at", { ascending: false }).limit(20);
        setSessions(sData || []);
    ...
    const sessCh = supabase.channel("sess-live")
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "drill_sessions", filter: `id=eq.${session.id}` },
        payload => {
          ...
          if (!payload.new.active) setActiveSession(null);
        })
      .subscribe();
  ...
  const startDrill = async () => {
    ...
    setSession(data); setActiveSession(data); setAtt({});
    setSessions(prev => [data, ...prev]);
    setTab("mine"); setPage("drill");
  };

  // ── Join drill ────────────────────────────────────────────────────────────
  const joinDrill = async (sess) => {
    if (!myMarshal) return;
    setSession(sess);
  ...
  const stopDrill = async () => {
    ...
    if (data) { setSession(data); setActiveSession(null); }
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg,#0f172a,#1e293b)", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 40px" }}>
        <div style={{ padding: "16px 0 12px", display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={() => setPage("setup")} style={{ background: "rgba(255,255,255,0.08)", border: "none", color: "#94a3b8", borderRadius: 8, padding: "6px 12px", fontSize: 13 }}>← Back</button>
        ...
        <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 14, padding: 16, marginTop: 20, border: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <input className="inp" value={newMarshalName} onChange={e => setNewMarshalName(e.target.value)} onKeyDown={e => e.key === "Enter" && addMarshal()} placeholder="Full name"
      ...
      <div style={{ width: "100%", maxWidth: 480, padding: "28px 16px 40px" }}>
        ...
        {activeSession && (
          <div style={{ background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.3)", borderRadius: 12, padding: "12px 16px", marginBottom: 12 }}>
            <div style={{ color: "#fbbf24", fontSize: 13, fontWeight: 700, marginBottom: 4 }}>⚡ Active drill in progress</div>
            <div style={{ color: "#d97706", fontSize: 12 }}>Started by <strong>{activeSession.started_by}</strong> · {fmtTime(activeSession.started_at)}</div>
            <button onClick={() => joinDrill(activeSession)} disabled={!myMarshal}
              style={{ width: "100%", marginTop: 10, padding: 11, background: myMarshal ? "#d97706" : "#374151", color: "white", border: "none", borderRadius: 9, fontWeight: 700, fontSize: 15, opacity: myMarshal ? 1 : 0.5 }}>
              Join Active Drill
            </button>
          </div>
        )}
        ...
        <button onClick={startDrill} disabled={!myMarshal}
          style={{ width: "100%", padding: 14, background: myMarshal ? "#dc2626" : "#1e293b", color: "white", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 800, marginBottom: 10, transition: "all .15s", opacity: myMarshal ? 1 : 0.5, boxShadow: myMarshal ? "0 4px 20px rgba(220,38,38,0.35)" : "none" }}>
          🚨 {activeSession ? "Start New Drill (Reset)" : "Start Fire Drill"}
        </button>
        ...
        {sessions.filter(s => !s.active).length > 0 && (
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 14, padding: 14, marginBottom: 12, border: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ color: "#64748b", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 10 }}>Past Sessions</div>
            {sessions.filter(s => !s.active).slice(0, 5).map(s => (
              <div key={s.id} style={{ display: "flex", alignItems: "center", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.04)", gap: 10 }}>
                <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#334155", flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <div style={{ color: "#94a3b8", fontSize: 13 }}>{fmtDate(s.started_at)} · {fmtTime(s.started_at)}</div>
                  <div style={{ fontSize: 11, color: "#475569" }}>{s.started_by} · {elapsed(s.started_at, s.ended_at)}</div>
                </div>
                <button onClick={() => { setSession(s); loadAttendance(s.id).then(() => { setTab("mine"); setPage("drill"); }); }} disabled={!myMarshal}
                  style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "#94a3b8", borderRadius: 7, padding: "4px 10px", fontSize: 11, opacity: myMarshal ? 1 : 0.4 }}>
                  View
                </button>
              </div>
            ))}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setPage("marshals")} style={{ flex: 1, padding: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", color: "#64748b", borderRadius: 10, fontSize: 13 }}>🧑‍🚒 Marshals</button>
          <button onClick={() => { setEmpSearch(""); setPage("employees"); }} style={{ flex: 1, padding: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", color: "#64748b", borderRadius: 10, fontSize: 13 }}>👥 Employees</button>
        </div>
      </div>
    </div>
  );
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 10, fontFamily: "'DM Mono', monospace", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {drillEnded ? `Ended ${fmtTime(session.ended_at)} · ${session.ended_by}` : `${elapsedStr} · ${myMarshal?.name}`}
            </div>
          </div>
          ...
          <button onClick={() => { setPage("setup"); setSession(null); setAtt({}); }} style={{ background: "rgba(255,255,255,0.08)", border: "none", color: "rgba(255,255,255,0.5)", borderRadius: 8, padding: "6px 10px", fontSize: 12 }}>⚙</button>
        </div>
      ...
      <div style={{ padding: "8px 10px 110px" }}>
        {visEmps.length === 0 && (
          <div style={{ textAlign: "center", color: "#94a3b8", padding: "40px 20px", fontSize: 14 }}>
            {tab === "mine" && myParty.length === 0 ? "No employees assigned to your party yet.\nGo to Employees to assign them." : "No employees found."}
          </div>
        )}
      ...
      {confirmStop && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }} onClick={() => setConfirmStop(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: 20, padding: "28px 24px", width: "100%", maxWidth: 360, textAlign: "center" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 20, color: "#0f172a", marginBottom: 8 }}>End Drill?</div>
            <div style={{ color: "#64748b", fontSize: 14, marginBottom: 12 }}>The end time will be recorded in the report.</div>
