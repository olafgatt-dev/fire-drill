        ...
        {pastSessions.map(s => (
          <div key={s.id} className="fade-in" style={{ background: "rgba(255,255,255,0.05)", borderRadius: 14, padding: "14px 16px", marginBottom: 10, border: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={() => {
                  if (!myMarshal) { alert("Select your name on the home screen first."); return; }
                  setSession(s);
                  loadAttendance(s.id).then(() => { setTab("mine"); setPage("drill"); });
                }}
              ...
              <button
                ...
                style={{ width: 44, padding: "10px 0", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", color: "#f87171", borderRadius: 9, fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>
                🗑
              </button>
            </div>
          </div>
  ...
  if (page === "setup") return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg,#0f172a 0%,#1e293b 100%)", fontFamily: "'DM Sans', system-ui, sans-serif", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: 480, padding: "28px 16px 40px" }}>
        ...
        {/* Nav buttons */}
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setPage("marshals")} style={{ flex: 1, padding: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", color: "#64748b", borderRadius: 10, fontSize: 13 }}>
            🧑‍🚒 Marshals
          </button>
          <button onClick={() => { setEmpSearch(""); setPage("employees"); }} style={{ flex: 1, padding: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", color: "#64748b", borderRadius: 10, fontSize: 13 }}>
            👥 Employees
          </button>
          <button onClick={() => setPage("sessions")} style={{ flex: 1, padding: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", color: "#64748b", borderRadius: 10, fontSize: 13, position: "relative" }}>
            📋 History
            {pastSessions.length > 0 && (
