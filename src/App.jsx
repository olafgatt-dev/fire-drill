...
export default function App() {
  ...
  useEffect(() => {
    ...
    const goOnline  = async () => {
      ...
      // Re-fetch session and attendance to catch anything missed while offline
      if (session) {
...
function ConnBar({ status }) {
  ...
  const cfg = {
    offline: { bg: "#7f1d1d", border: "#991b1b", dot: "#f87171", label: "Offline — changes won't sync", animate: false },
    syncing: { bg: "#1c3454", border: "#1d4ed8", dot: "#60a5fa", label: "Reconnected — syncing…",       animate: true  },
  }[status];
  return (
    <div style={{ background: cfg.bg, borderBottom: `1px solid ${cfg.border}`, padding: "5px 14px", display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "white", fontWeight: 600, zIndex: 30 }}>
      <div style={{ width: 8, height: 8, borderRadius: "50%", background: cfg.dot, flexShrink: 0, animation: cfg.animate ? "syncFlash 0.8s ease infinite alternate" : "none" }} />
      {cfg.label}
    </div>
  );
}
