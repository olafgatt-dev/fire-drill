...
export default function App() {
  ...
  // ── Add temp person during active drill ───────────────────────────────────
  ...
  // ── Print report ──────────────────────────────────────────────────────────
  const handlePrint = () => {
    // Only include permanent staff + temp staff created in this session
    ...
    if (unassigned.length > 0) {
      ...
      unassigned.forEach(e => {
        const r = att[e.id]; const st = r?.status || "unaccounted";
        const temp = e.is_temp ? "  [TEMP]" : "";
        lines.push(`  ${STATUS_META[st].icon} ${e.name.padEnd(22)} ${e.dept || ""}${temp}`);
      });
      lines.push("");
    }
...
@media print{body{padding:16px;font-size:11px}}</style>
  ...
  // Employees visible in the current drill context (excludes other sessions' temp people)
        ...
        {marshals.map(m => (
          <div key={m.id} className="fade-in" style={{ display: "flex", alignItems: "center", background: "rgba(255,255,255,0.05)", borderRadius: 12, padding: "12px 16px", marginBottom: 8, border: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: "#475569", fontSize: 11, marginTop: 1 }}>{employees.filter(e => e.marshal_id === m.id && !e.is_temp).length} people in party</div>
            </div>
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg,#0f172a,#1e293b)", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 40px" }}>
        <div style={{ padding: "16px 0 12px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ marginLeft: "auto", color: "#475569", fontSize: 13 }}>{employees.filter(e => !e.is_temp).length} total</div>
        </div>
              ...
              return (
                <button key={m.id} onClick={() => setMyMarshal(m)}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: "#475569", fontSize: 11, marginTop: 1 }}>Party of {employees.filter(e => e.marshal_id === m.id && !e.is_temp).length}</div>
                  </div>
    ...
    <div className="emp-row fade-in" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 7, background: "white", borderRadius: 12, padding: "10px 12px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", border: status === "missing" ? "2px solid #fca5a5" : "2px solid transparent" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12, color: "#64748b", marginTop: 1, display: "flex", alignItems: "center", gap: 5 }}>
          {emp.dept || "—"}
          {emp.is_temp && (
            <span style={{ background: "#fef3c7", color: "#92400e", borderRadius: 4, padding: "1px 5px", fontSize: 10, fontWeight: 700 }}>TEMP</span>
          )}
        </div>
        {hasNote && (
